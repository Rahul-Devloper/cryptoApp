...
const CryptoDetails = () => {
  ...
  const { '24hVolume': volume } = cryptoDetails
  console.log('volume=>', volume)
  //below is the metaDeta (the ui will form from this data)
      ...
      <Col className='coin-desc-link'>
        ...
        <Col className='coin-links'>
          ...
          {cryptoDetails?.links.map((link) => (
            <Row key={link} className='coin-link'>
              <Title level={5} className='link-name'>
                {link?.type}
              </Title>
              <a href={link.url} target='_blank' rel='no-refer'>
                {link.name}
              </a>
            </Row>
          ))}
